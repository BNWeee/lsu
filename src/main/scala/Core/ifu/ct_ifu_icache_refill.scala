...
class ct_ifu_icache_refill extends Module with Config with CacheConfig {
  ...
  val SRam_write  =RegNext(VecInit(Seq.fill(RefillTimes)(0.U(RamhelperBits.W)))) //往


  //L2测试可先用ramhelper
  when(state === s_pf0 && io.req.valid) {
    ram.io.rIdx := (paddr - pc_reset(PC_WIDTH - 1, 0)) >> 3.U //64bit一跳转//第一拍直接以请求当拍的paddr读，第二拍开始是用寄存器地址
    SRam_write(0)  := ram.io.rdata
  }.elsewhen(state === s_pf0_done){
    ram.io.rIdx := (paddrReg - pc_reset(PC_WIDTH - 1, 0) + 1.U) >> 3.U
    SRam_write(1)  := ram.io.rdata
  }.elsewhen(state === s_pf1){
    ram.io.rIdx := (paddrReg - pc_reset(PC_WIDTH - 1, 0) + 2.U) >> 3.U
    SRam_write(0)  := ram.io.rdata
  }.elsewhen(state === s_pf1_done){
    ram.io.rIdx := (paddrReg - pc_reset(PC_WIDTH - 1, 0) + 3.U) >> 3.U
    SRam_write(1)  := ram.io.rdata
  }.elsewhen(state === s_pf2){
    ram.io.rIdx := (paddrReg - pc_reset(PC_WIDTH - 1, 0) + 4.U) >> 3.U
    SRam_write(0)  := ram.io.rdata
  }.elsewhen(state === s_pf2_done){
    ram.io.rIdx := (paddrReg - pc_reset(PC_WIDTH - 1, 0) + 5.U) >> 3.U
    SRam_write(1)  := ram.io.rdata
  }.elsewhen(state === s_pf3){
    ram.io.rIdx := (paddrReg - pc_reset(PC_WIDTH - 1, 0) + 6.U) >> 3.U
    SRam_write(0)  := ram.io.rdata
  }.elsewhen(state === s_pf3_done){
    ram.io.rIdx := (paddrReg - pc_reset(PC_WIDTH - 1, 0) + 7.U) >> 3.U
    SRam_write(1)  := ram.io.rdata
  }
