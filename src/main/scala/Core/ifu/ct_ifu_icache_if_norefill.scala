...
class ICache(cacheNum: Int =0, is_sim: Boolean) extends Module with Config with CacheConfig {
  ...
  val SRam_read = Seq.fill(CacheCatNum)(WireInit(VecInit(Seq.fill(Banks)(0.U(sram_width.W)))))//4次取，bank表数目，每表32bit
  val SRam_write  = Seq.fill(CacheCatNum)(WireInit(VecInit(Seq.fill(Banks)(0.U(sram_width.W))))) //4次取，bank表数目，每表32bit
  ...
  val storeEn     = io.cache_req.fire() && (state === s_idle || state === s_lookUp) //流水线请求有效并且处于idle或Cache访问请求寻址状态
  val reqValid    = RegInit(false.B) // Reg(Vec(2, Bool())) //用寄存器记录两条指令内存请求有效
  val paddrReg = RegInit(0.U(PC_WIDTH.W))
  val vaddrReg = RegInit(0.U(PC_WIDTH.W))
  val tagReg = ftag(paddrReg)
  val indexReg = ftag(vaddrReg)
  ...
  val refillDataReg = RegInit(VecInit(Seq.fill(CacheCatNum)(0.U(axiDataBits.W))))  //用来寄存读取内存的数据, 4*128bit, 64 byte
...
//  val wb_data = Reg
  ...
  when(predec_times < RetTimes.U && state === s_refill_done){
    when(fifo === 0.U) {
      ...
      predecd_array(0).io.idx := Mux(predec_write, Drefill_idx, DreadIdx)
      ...
      predecd_array(0).io.wen := fw_write || refill_write
    }.otherwise {
      icache_predec.io.din := refillDataReg(predec_times)
      predec_dout(predec_times) := icache_predec.io.dout
      predecd_array(1).io.idx := Mux(predec_write, Drefill_idx, DreadIdx)
      ...
      predecd_array(1).io.wen := predec_write
    }
    predec_times := predec_times + 1.U(log2Up(RetTimes).W)
  }


  for (i <- 0 until Ways) {
    tagHitVec(i) := io.cache_req.fire() && valid(i)(index) && tag_array(i)(index) === tag
    //validVec(i) := valid(i)(indexReg)//似乎并没有用到
  }
  fifo := fifo_array(indexReg)
  hit := tagHitVec.asUInt.orR //每一拍都直接辨别当拍是否hit
  hitReg := hit
  when(storeEn) {
    reqValid := io.cache_req.fire() //当流水线请求任一条有效时，更新为refill准备的寄存器里的请求有效
    paddrReg := paddr
    vaddrReg := vaddr
    needRefill(fifo)(refill_idx) := io.cache_req.fire() && !hit
  }
  ...
  for (i <- 0 until  CacheCatNum) {
    when(state === s_miss && io.cohresp.valid && io.cohresp.bits.needforward){ //cache miss并且响应有效，且响应需要向前
      tag_array(fifo)(refill_idx) := tagReg
      valid(fifo)(refill_idx)    := true.B
  ...
  when(axireadMemCnt < RetTimes.U && state === s_refill){
    when(needRefill(fifo)(indexReg) && refill_ready){
  ...
  DreadIdx := Mux(state===s_refill_done,DindexReg,Dindex)

  for( i<- 0 until  CacheCatNum){//这里先按C910考虑sram一次读写128bit
    SRam_read(i) := Mux(fifo === 0.U,SRamArray(0).io.rData.asTypeOf(SRam_read(i)),SRamArray(1).io.rData.asTypeOf(SRam_read(i)))
    reqreadReg(i) := SRam_read(i).asUInt() //读Cache Data寄存器更新，放到寄存器里
    reqreadPredecReg(i) := Mux(fifo===0.U,predecd_array(0).io.rData,predecd_array(1).io.rData)
    ...
    when(fifo ===0.U ){
      SRamArray(0).io.idx := Mux(refill_write || fw_write, Drefill_idx, DreadIdx) + i.U
      SRamArray(0).io.wMask := VecInit(Seq.fill(128)(true.B)).asUInt() //wmask had been done
      SRamArray(0).io.wData := SRam_write(i).asUInt()
      SRamArray(0).io.en := hit_read || refill_write || fw_write
      SRamArray(0).io.wen := fw_write || refill_write
    }.otherwise{
      SRamArray(1).io.idx := Mux(refill_write || fw_write, Drefill_idx, DreadIdx) + i.U
      SRamArray(1).io.wMask := VecInit(Seq.fill(128)(true.B)).asUInt() //wmask had been done
      SRamArray(1).io.wData := SRam_write(i).asUInt()
      SRamArray(1).io.en := hit_read || refill_write || fw_write
      SRamArray(1).io.wen := fw_write || refill_write
    }
  }


  stateReg1 := state === s_predec_done  //每一拍都侦听是否完成refill，存一拍
  ...
  io.cache_resp.valid := ( hitReg ||//直接命中情况，至少需要一拍读sram
  ...
  switch(state) {
    ...
    is(s_miss) {
      when(io.cohresp.valid || io.cohresp.bits.needforward){//todo:假定needforward保持为false,一进dcache若所需存在dcache立即置为true
        ...
        state := s_refill
        valid(fifo)(refill_idx) := false.B
      }
    }
    is(s_cohresp){
      when(io.cohresp.valid) {
        when(io.cohresp.bits.needforward && needRefill(fifo)(refill_idx)) {
          state := s_cohresp
          needRefill(fifo)(refill_idx) := false.B //其它缓存响应了不需要refill了
        }.elsewhen(!io.cohresp.bits.needforward) {
          needRefill(fifo)(refill_idx) := false.B //其它缓存响应了不需要refill了
          state := s_refill_done
        }
      }
    ...
    is(s_refill) {
      when((axireadMemCnt === RetTimes.U) && needRefill(fifo)(refill_idx)){
        state := s_refill_done
        needRefill(fifo)(refill_idx) := false.B
        predec_times := 0.U
